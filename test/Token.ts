...
describe("Juicy Token", () => {
  describe("Initiation", () => {
    ...
    it("should deploy contract", async () => {
      JuicyToken = await (
        await ethers.getContractFactory("JuicyToken")
      ).deploy(INITIAL_SUPPLY, MAXIMUM_TOTAL_SUPPLY, INITIAL_MULTIPLIER);
      ...
      console.log(
        `Current multiplier is x${Number(await JuicyToken.getCurrentMultiplier()) / 1e4}`
      );
    });

    it("should deploy test contracts", async () => {
      TransferMock1 = await (
        await ethers.getContractFactory("TransferContract")
  ...
  describe("Transfers", () => {
    it("should time pass", async () => {
      await mine(SECONDS_IN_YEAR / BLOCK_DURATION + 1, {
        interval: BLOCK_DURATION,
      });
    });
    ...
    it("should user transfer tokens wallet-to-wallet", async () => {
      ...
      expect(await JuicyToken.totalSupply()).closeTo(toBig(1_500_000), toBig(1000));
      expect(await JuicyToken.getCurrentMultiplier()).closeTo(14375, 2);
      expect(await JuicyToken.balanceOf(owner.address)).closeTo(toBig(1_499_000), toBig(1000));
      expect(await JuicyToken.balanceOf(addr1.address)).eq(toBig(1000));
    });
    ...
    it("should user transfer tokens from wallet to contract", async () => {
      ...
      expect(await JuicyToken.balanceOf(TransferMock1.target)).eq(toBig(10000));
    });
    ...
    it("should user transfer tokens from contract to wallet", async () => {
      ...
      expect(await JuicyToken.balanceOf(TransferMock1.target)).eq(toBig(9000));
    });

    it("should user transfer tokens from contract to wallet", async () => {
      await transferContractToWallet(JuicyToken, TransferMock1, addr2, 1000);

      expect(await JuicyToken.balanceOf(TransferMock1.target)).eq(toBig(8000));
      expect(await JuicyToken.balanceOf(addr2.address)).eq(toBig(1000));
    });

    it("should user transfer tokens from wallet to contract", async () => {
      await transferWalletToContract(JuicyToken, owner, TransferMock1, 2_100_000);

      expect(await JuicyToken.balanceOf(TransferMock1.target)).eq(toBig(2_108_000));
    });
    ...
    it("should user transfer tokens from contract to contract", async () => {
      ...
      expect(await JuicyToken.balanceOf(TransferMock1.target)).eq(toBig(2_008_000));
      expect(await JuicyToken.balanceOf(TransferMock2.target)).eq(toBig(100_000));
    });

    it("should user transfer tokens from contract to wallet", async () => {
      await transferContractToWallet(JuicyToken, TransferMock1, addr3, 2000_000);

      expect(await JuicyToken.balanceOf(TransferMock1.target)).eq(toBig(8_000));
      expect(await JuicyToken.balanceOf(addr3.address)).eq(toBig(2000_000));
    });
